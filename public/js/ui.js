...
export const UI = {
    ...
    focusInput() { this.elements.input.focus(); },
    getInputText() { return this.elements.input.value.trim(); },

    // ==================== Typing indicator ====================

    showTyping(visible) { this.elements.typing.classList.toggle('visible', visible); },
    ...
    setSelectedModel(modelId) { this.elements.modelSelect.value = modelId; },
    ...
    renderPreviews(attachments) {
        ...
        if (attachments.length === 0) { area.style.display = 'none'; return; }
        ...
        if (role === 'user') {
            ...
            if (attachments?.length > 0) {
                ...
                for (const att of attachments) {
                    if (att.type === 'image') inner += `<img class="msg-thumb" src="${att.data}" alt="">`;
                    else inner += `<span class="msg-file">📄 ${att.name}</span>`;
                }
                inner += '</div>';
            }
    ...
    clearChat() { this.elements.chat.innerHTML = ''; },

    scrollToBottom() { this.elements.chat.scrollTop = this.elements.chat.scrollHeight; },
    ...
    enterEditMode(msgDiv, originalText, onSave, onCancel) {
        ...
        saveBtn.onclick = () => { const newText = ta.value.trim(); if (newText) onSave(newText); };
