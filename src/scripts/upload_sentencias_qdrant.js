/**
 * upload_sentencias_qdrant.js — Upload sentencias corpus to Qdrant collection "sentencias".
 *
 * Uses one point per sentencia (full text in payload).
 * Dense embedding text: abstract + palabras_clave.
 * Sparse vector: precomputed in sentencias_sparse_vectors.json.
 *
 * Usage:
 *   node src/scripts/upload_sentencias_qdrant.js --limit 200 --wipe
 *   node src/scripts/upload_sentencias_qdrant.js --resume
 */
...
const DATA_PATH = path.join(ROOT, 'data', 'chunks', 'sentencias_enriched.json');
const SPARSE_PATH = path.join(ROOT, 'data', 'chunks', 'sentencias_sparse_vectors.json');
const PROGRESS_PATH = path.join(ROOT, 'data', 'chunks', 'sentencias_upload_progress.json');
...
function buildEmbeddingText(item) {
    ...
    const fallback = [item.title || '', (item.text || '').slice(0, 1200)].join('\n').trim();
...
function parseNumericId(item, fallbackIndex) {
    const fromId = Number(item.sentencia_id);
...
async function main() {
    ...
    console.log('=== Upload Sentencias to Qdrant ===\n');
        ...
        try {
            ...
            const points = batchItems.map((item, idx) => {
                ...
                return {
                    ...
                    payload: {
                        // Compatibility with current backend model mapping
                        law: item.title || '',
                        chapter: item.date || '',
                        section: 'Sentencia TS',
                        text: item.text || '',
                        ...
                        // Sentencias-specific
                        sentencia_id: item.sentencia_id || null,
                        date: item.date || '',
                        fallo: item.fallo || '',
