...
// Stream an HTTPS request — returns the raw response for piping
...
// ── Step 2: Search Azure AI Search (hybrid: vector + text + semantic) ──
...
function buildContext(results) {
    ...
    return results.map((doc, i) => {
        const parts = [];
        parts.push(`[Fuente ${i + 1}]`);
        parts.push(`Ley: ${doc.law}`);
...
// ── Step 4: Call GPT-5.2 with context and stream response ──
async function callGPTStream(context, messages) {
    // Build the augmented messages
    ...
    const recentHistory = messages.slice(-6); // last 3 turns (user+assistant pairs)
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        // Build the sources metadata to append at the end
        ...
        // For Azure Functions v3, we can't truly stream, so we collect and return
